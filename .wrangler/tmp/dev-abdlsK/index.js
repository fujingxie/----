...
// .wrangler/tmp/bundle-LEgwlY/checked-fetch.js
...
async function ensureClassSettings(db, classId) {
  ...
  await db.prepare("INSERT INTO class_settings (class_id, level_thresholds) VALUES (?, ?)").bind(classId, JSON.stringify(DEFAULT_LEVEL_THRESHOLDS)).run();
}
__name(ensureClassSettings, "ensureClassSettings");
async function getUserById(db, userId) {
  const user = await db.prepare("SELECT id, username, nickname, level, expire_at FROM users WHERE id = ?").bind(userId).first();
  if (!user) {
...
async function appendLog(db, { classId, userId, actionType, detail }) {
  await db.prepare("INSERT INTO logs (class_id, user_id, action_type, detail) VALUES (?, ?, ?, ?)").bind(classId, userId, actionType, detail).run();
}
...
async function handleLogin(db, request) {
  ...
  let user = await db.prepare("SELECT id, username, nickname, level, expire_at FROM users WHERE username = ?").bind(username).first();
  if (!user) {
    const inserted = await db.prepare(
      `INSERT INTO users (username, password_hash, nickname, level)
         VALUES (?, ?, ?, ?) RETURNING id, username, nickname, level, expire_at`
    ).bind(username, "demo-password", nickname, "permanent").first();
    user = inserted;
...
// .wrangler/tmp/bundle-LEgwlY/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-LEgwlY/middleware-loader.entry.ts
