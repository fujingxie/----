...
// .wrangler/tmp/bundle-z67Z2y/checked-fetch.js
...
var getTodayKeyCN = /* @__PURE__ */ __name(() => new Intl.DateTimeFormat("en-CA", {
  timeZone: "Asia/Shanghai",
  year: "numeric",
  month: "2-digit",
  day: "2-digit"
}).format(/* @__PURE__ */ new Date()), "getTodayKeyCN");
...
var normalizeStudentLog = /* @__PURE__ */ __name((row) => ({
  id: Number(row.id),
  action: row.action,
...
__name(formatUtcMsToDbTimestamp, "formatUtcMsToDbTimestamp");
...
async function getLastBulkFedAt(db, classId) {
  await ensureClassSettings(db, classId);
  const settings = await db.prepare("SELECT last_bulk_fed_at FROM class_settings WHERE class_id = ?").bind(classId).first();
  return settings?.last_bulk_fed_at || null;
}
__name(getLastBulkFedAt, "getLastBulkFedAt");
...
async function appendStudentLog(db, { classId, studentId, action, ruleName, ruleIcon, expDelta, coinsDelta, expAfter, coinsAfter, levelAfter }) {
  await db.prepare(
    `INSERT INTO student_logs
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(classId, studentId, action, ruleName, ruleIcon, expDelta, coinsDelta, expAfter, coinsAfter, levelAfter).run();
}
__name(appendStudentLog, "appendStudentLog");
async function getStudentLogsByStudentId(db, classId, studentId, limit = 50, offset = 0) {
  const result = await db.prepare(
    `SELECT id, action, rule_name, rule_icon, exp_delta, coins_delta, exp_after, coins_after, level_after, created_at
       ...
       ORDER BY created_at DESC, id DESC
       LIMIT ? OFFSET ?`
  ).bind(classId, studentId, limit, offset).all();
  const countResult = await db.prepare("SELECT COUNT(*) as total FROM student_logs WHERE class_id = ? AND student_id = ?").bind(classId, studentId).first();
  return {
    logs: (result.results || []).map(normalizeStudentLog),
    total: Number(countResult?.total || 0)
  };
}
__name(getStudentLogsByStudentId, "getStudentLogsByStudentId");
...
async function applyFeedToStudents(db, classId, userId, studentIds, selectedRule = null, options = { isDailyBulkFeed: false }) {
  ...
  const interactionRule = {
    ...
    icon: String(selectedRule?.icon || (options.isDailyBulkFeed ? "\u{1F357}" : "\u2728")),
    exp: Number(selectedRule?.exp ?? 1),
    coins: Number(selectedRule?.coins || 0),
    type: selectedRule?.type === "negative" ? "negative" : "positive"
  };
  const logActionType = options.isDailyBulkFeed ? "bulk_feed" : studentIds.length > 1 ? "bulk_interact" : "interact";
  for (const row of rows) {
...
__name(handleGetStudentLogs, "handleGetStudentLogs");
...
__name(handleGetProgressRanking, "handleGetProgressRanking");
...
async function handleFeedStudentsBatch(db, request, classId) {
  ...
  if (isDailyBulkFeed) {
    ...
    const lastBulkFedAt2 = await getLastBulkFedAt(db, classId);
    if (lastBulkFedAt2) {
      const lastFedDateKey = new Intl.DateTimeFormat("en-CA", {
        ...
        month: "2-digit",
        day: "2-digit"
      }).format(new Date(lastBulkFedAt2));
  ...
  if (isDailyBulkFeed) {
    await ensureClassSettings(db, classId);
    await db.prepare("UPDATE class_settings SET last_bulk_fed_at = ? WHERE class_id = ?").bind(nowIso(), classId).run();
  }
...
// .wrangler/tmp/bundle-z67Z2y/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-z67Z2y/middleware-loader.entry.ts
